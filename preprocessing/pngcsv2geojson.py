...
# import sys
# sys.path.append(r"..")
from utils.preprocessing import get_names, create_dir, parse_path, format_contour, create_geojson, read_labels, save_geojson, read_labels_idx
...
def pngcsv2geojson(png: np.ndarray, csv: pd.DataFrame, num_classes: Optional[int] = 2) -> List[Dict[str, Any]]:
    ...
    for _, (idx, cell_label) in csv.iterrows():
        ...
        # External contours from binary image
        mask = png.copy()
        mask[mask != idx] = 0
        mask[mask == idx] = 1
