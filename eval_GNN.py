...
from preprocessing import pngcsv2centroids_main
...
def run_preprocessing(args: Namespace, logger: Logger) -> None:
    ...
    for split in ['train', 'validation', 'test']:
        ...
        newargs = Namespace(
            png_dir=os.path.join(args.root_dir, 'data', split, 'png_gnn'+args.data_suffix),
            csv_dir=os.path.join(args.root_dir, 'data', split, 'csv_gnn'+args.data_suffix),
            output_path=os.path.join(args.root_dir, 'data', split, 'centroids_gnn'+args.data_suffix),
            eval=True
        )
...
def run_evaluation(args: Namespace, logger: Logger) -> None:
    ...
    for split in ['train', 'validation', 'test']:
        ...
        if args.debug:
            os.makedirs(os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'gnn_individual'), exist_ok=True)
        
        newargs = Namespace(
            names=names,
            gt_path=os.path.join(args.root_dir, 'data', split, 'centroids'),
            pred_path=os.path.join(args.root_dir, 'data', split, 'centroids_gnn'+args.data_suffix),
            save_name=os.path.join(args.save_dir, 'gnn', args.data_suffix, split),
            debug_path=os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'gnn_individual',
        ...
        if args.debug:
            ...
            shutil.move(
                os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'gnn_individual', 'debug_gnn_global_nobkgr.csv'),
                os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'debug_gnn_global_nobkgr_' + split + '.csv'))

            shutil.move(
                os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'gnn_individual', 'debug_gnn_global_nobkgr.png'),
                os.path.join(args.save_dir, 'gnn', args.data_suffix, split, 'conf-matrices', 'debug_gnn_global_nobkgr_' + split + '.png'))
...
def _create_parser():
    ...
    parser.add_argument('--data-suffix', type=str, default="", help="Suffix of the folders to load the result.")
