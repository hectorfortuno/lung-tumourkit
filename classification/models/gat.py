...
class GAT(nn.Module):
    def __init__(self, in_size, hid_size, out_size, heads, num_layers, dropout, norm_type, enable_background=False):
        ...
        self.gat_layers.append(GATConv(in_size, hid_size, heads[0],
        ...
        for l in range(num_layers-1):
            self.gat_layers.append(GATConv(hid_size * heads[l], hid_size, heads[l+1],
                                           feat_drop=dropout, attn_drop=dropout, activation=F.elu,))
            self.gat_layers.append(Norm(norm_type=norm_type, hidden_dim=hid_size * heads[l+1]))

        self.gat_layers.append(GATConv(hid_size * heads[-2], out_size, heads[-1],
                                       feat_drop=dropout, attn_drop=dropout, activation=None,))

        self.enable_background = enable_background
        if enable_background:
            self.bkgr_head = GATConv(hid_size * heads[-2], 2, heads[-1],
                                     feat_drop=dropout, attn_drop=dropout, activation=None,)
    ...
    def forward(self, g, inputs):
        ...
        for i, layer in enumerate(self.gat_layers):
            if i == len(self.gat_layers)-1:  # last layer: mean over the heads
                if self.enable_background:
                    h_bkgr = self.bkgr_head(g, h).mean(1)
                    h = layer(g, h).mean(1)
                    return h, h_bkgr
                h = layer(g, h).mean(1)
            else:  # other layer(s): concatenate features of all the heads
                h = layer(g, h)
                h = h.flatten(1)
        return h