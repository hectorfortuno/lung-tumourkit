...
from preprocessing import pngcsv2graph_main, pngprob2graph_main
...
def draw_graph(
    ...
    gx = torch_geometric.utils.to_networkx(graph, to_undirected=True)
    ...
    fig.set_size_inches(4,4); fig.tight_layout()

    ax = plt.Axes(fig, [0., 0., 1., 1.]); ax.set_axis_off(); fig.add_axes(ax)

    nx.draw_networkx(gx, pos=pos, arrows=False, node_color=cols, with_labels=False, node_size=2, width=0.25, ax=ax)
...
def main_subthread(
    ...
    X, y, xx, yy = read_node_matrix(
        ...
        remove_prior=False, remove_area=False, remove_coords=False, remove_hist=False, remove_perimeter=False, remove_std=False
        )
...
def main_with_args(args: Namespace) -> None:
    ...
    names = sorted(get_names(os.path.join(args.output_dir, 'graphs'), '.nodes.csv'))
