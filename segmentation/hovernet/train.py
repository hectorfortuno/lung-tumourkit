...
class TrainManager(Config):
    ...
    def _get_datagen(self, batch_size, run_mode, target_gen, nr_procs=0, fold_idx=0, stroma_mask=False):
    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            if pretrained_path is not None:
                ...
                # Load specific checkpoint (initial weights)
                else:
    ...
    def run(self):
        ...
        # Run in different phases
        prev_save_path = None
        for phase_idx, phase_info in enumerate(phase_list):

            # Save logs phases
            if len(phase_list) == 1: save_path = self.log_dir
            else: save_path = self.log_dir + "/%02d/" % (phase_idx)
...
def main_with_args(args):
    trainer = TrainManager(
        args.shape, args.log_dir, args.train_dir, args.valid_dir, args.pretrained_path,
        use_cpu=(args.gpu == ''), num_classes=args.num_classes, stroma_mask=args.stroma_mask
    )
