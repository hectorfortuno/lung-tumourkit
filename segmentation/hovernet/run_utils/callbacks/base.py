...
class ConditionalSaver(BaseCallbacks):
    ...
    def run(self, state, event):
        ...
        # Best value of the previous epochs
        ...
        # Compare previous best value with the current one
        ...
        # Save checkpoint current epoch as the best one
        for net_name, net_info in state.run_info.items():
            net_checkpoint = {}
            for key, value in net_info.items():
                if key != "extra_info" and key != "use_cpu":
                    net_checkpoint[key] = value.state_dict()
            torch.save(net_checkpoint, "%s/%s_best=[%s].tar" % (state.log_dir, net_name, self.metric_name),)
