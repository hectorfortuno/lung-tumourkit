...
def train_step(batch_data, run_info):
    ...
    # True values, "ground truth" of the Nuclear Pixel and the HoVer branches
    ...
    # True values, "ground truth" of the Nuclear Type Classification branch
    ...
    for branch_name in pred_dict.keys():
        for loss_name, loss_weight in loss_opts[branch_name].items():
            ...
            if branch_name == "tp":
                label_smoothing = 0.1
                loss_args.append(label_smoothing)
                # Weights to balance cross entropy
                """if loss_name == "bce":
                    loss_args.append([0.05, 0.1, 0.85])"""
                # Weights to balance dice loss
                """if loss_name == "dice":
                    loss_args.append([0.05, 0.1, 0.85])"""
            term_loss = loss_func(*loss_args, device="cpu" if run_info["net"]["use_cpu"] else "cuda")
...
def valid_step(batch_data, run_info):
    ...
    # True and predicted values to compute metrics in next steps
...
####
def infer_step(batch_data, model, use_cpu=False):
    # Images
    ...
    model.eval()
    with torch.no_grad():  # dont compute gradient
        # Predictions of the model (HoverNet)
        pred_dict = model(patch_imgs_gpu)
        pred_dict = OrderedDict(
            [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict.items()]
        )

        # Output of the branches
        pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
        if "tp" in pred_dict:
            type_map = F.softmax(pred_dict["tp"], dim=-1)
            ...
            # Probabilities of class (softmax with temperature)
            T = 1
            prob_map = F.softmax(pred_dict["tp"] / T, dim=-1)
            prob_map = prob_map.type(torch.float32)

            # Original logits
            logits_map = pred_dict["tp"]
            logits_map = logits_map.type(torch.float32)
            
            pred_dict["tp"] = type_map

        pred_output = torch.cat(list(pred_dict.values()), -1)
        o1 = pred_output.cpu().numpy()
        o2 = prob_map.cpu().numpy()
        o3 = logits_map.cpu().numpy()
...
def proc_valid_step_output(raw_data, nr_types=None):
    ...
    # for idx in range(len(raw_data["prob_np"])):
    ...
    if nr_types is not None:
        ...
        for type_id in range(0, nr_types):
            ...
            over_total = 0
            for idx in range(len(raw_data["true_np"])):
            # for idx in range(len(raw_data["pred_tp"])):
    ...
    for idx in range(len(raw_data["true_np"])):
    # for idx in range(len(raw_data["pred_hv"])):
