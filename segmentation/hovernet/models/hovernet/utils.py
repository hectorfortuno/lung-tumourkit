...
####
def xentropy_loss(true, pred, label_smoothing=0.0, weights=None , reduction="mean", device="cuda"):
    ...
    if label_smoothing > 0.0:
        ...
        true = (1 - label_smoothing)*true + label_smoothing / num_classes
    ...
    if weights is not None:
        if device == "cuda":
            weights = torch.tensor(weights).unsqueeze(0).to("cuda")
        else:
            weights = torch.tensor(weights).unsqueeze(0)
        loss = -torch.sum((true * weights * torch.log(pred)), -1, keepdim=True)
    else:
...
####
def dice_loss(true, pred, label_smoothing=0.0, weights=None, smooth=1e-3, device="cuda"):
    ...
    if weights is not None:
        ...
        loss = loss * weights
    loss = torch.sum(loss) # TODO: Divide the loss by num_classess so that it is between 0 and 1
    return loss


####
def mse_loss(true, pred, device="cuda"):
...
def msge_loss(true, pred, focus, device="cuda"):
    ...
    def get_sobel_kernel(size, device="cuda"):
    ...
    ####
    def get_gradient_hv(hv, device="cuda"):
        """For calculating gradient."""
        kernel_h, kernel_v = get_sobel_kernel(5, device=device)
    ...
    true_grad = get_gradient_hv(true, device=device)
    pred_grad = get_gradient_hv(pred, device=device)
