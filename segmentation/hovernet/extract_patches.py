...
def save_batch(
    ...
    if shape == 'full':
        ...
        for i in [0, 256, 512, 768]:
            for j in [0, 256, 512, 768]:
                crop = arr[i:i+256, j:j+256,]
                np.save(save_dir + name + '(' + str(i) + ',' + str(j) + ').npy', crop)
    else:
...
def _create_parser():
    ...
    parser.add_argument('--shape', type=str, choices=['256', '270', '518', 'full'], required=True, help='Height and width of the saved arrays. If less than full, patching is applied.')
    return parser
