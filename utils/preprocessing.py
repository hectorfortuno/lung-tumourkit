...
def read_csv_idx(name: str, csv_dir: str) -> Tuple[np.ndarray, pd.DataFrame]:
    """
...
def read_csv_prob(name: str, csv_dir: str, num_classes: int) -> Tuple[np.ndarray, pd.DataFrame]:
    """
    Reads a CSV file from the specified directory and returns its contents as a Pandas DataFrame.
    :param name: The name of the CSV file (without extension).
    :type name: str
    ...
    This function reads a CSV file from the specified directory and returns its contents as a Pandas DataFrame.
    The function expects the CSV file to have a specific filename format: '<name>.prob.csv'.
    If the function is unable to read the CSV file or encounters an error, it returns `None`.
    """
    try:
        csv = pd.read_csv(os.path.join(csv_dir, name + '.prob.csv'), header=None)
        if num_classes == 2:
            csv.columns = ['prob0', 'prob1']
        return csv
    except Exception:
        return None
...
def read_labels_idx(name: str, png_dir: str, csv_dir: str) -> Tuple[np.ndarray, pd.DataFrame]:
    """
...
def read_labels(name: str, png_dir: str, csv_dir: str) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    :param png_dir: The directory containing the PNG file.
    :type png_dir: str
    :param csv_dir: The directory containing the CSV file (without instance index).
    :type csv_dir: str
    :return: A tuple containing the NumPy array of the PNG file and the Pandas DataFrame of the CSV file.
    :rtype: Tuple[np.ndarray, pd.DataFrame] or None
...
def read_npy(name: str, npy_dir: str) -> np.ndarray:
    """
    Reads a NPY file from the specified directory and returns its image data as a NumPy array.

    :param name: The name of the NPY file (without extension).
    :type name: str
    :param npy_dir: The directory containing the NPY file.
    :type npy_dir: str
    :return: A NumPy array containing the image data.
    :rtype: np.ndarray or None

    This function reads a NPY file from the specified directory and returns its image data as a NumPy array.
    The function expects the NPY file to have a specific filename format: '<name>.npy'.
    If the function is unable to read the NPY file or encounters an error, it returns `None`.
    """
    try:
        img = np.load(os.path.join(npy_dir, name + '.npy'))
        return img
    except Exception:
        return None
...
def create_geojson(contours: List[Tuple[int, int]], num_classes: Optional[int] = 2) -> List[Dict[str, Any]]:
    ...
    # Labels and associated colour (value not important)
...
def save_csv_prob(csv: pd.DataFrame, csv_path: str, name: str) -> None:
    """
    Saves the given CSV file to the specified directory with the specified name and a '.prob.csv' extension.
    ...
    :type name: str
    """
    csv.to_csv(os.path.join(csv_path, name + '.prob.csv'), index=False, header=False)
...
def save_pngcsv(png: np.ndarray, csv: pd.DataFrame, png_path: str, csv_path: str, name: str) -> None:
    ...
    The PNG file is saved with a '.GT_cells.png' extension, and the CSV file is saved with a '.prob.csv' extension.
    """
    save_png(png, png_path, name)
    save_csv_prob(csv, csv_path, name)
...
def read_image(name: str, path: str) -> np.array:
    ...
    :rtype: np.array
    """
    aux = cv2.imread(os.path.join(path, name + '.png'), cv2.IMREAD_COLOR)
...
def extract_features(msk_img: np.ndarray, bin_msk: np.ndarray, debug=False) -> Dict[str, np.ndarray]:
    ...
    # Area and perimeter of the cell (in pixels)
    feats['area'] = bin_msk.sum()

    if debug:
        import pdb
        pdb.set_trace()
    contours, _ = cv2.findContours((bin_msk * 255).astype(np.uint8), mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE)
    ...
    gray_msk = ma.masked_array(gray_msk, mask=1 - bin_msk)
    feats['std'] = gray_msk.std()

    # Histogram RGB colour channels
    msk_img = ma.masked_array(msk_img, mask=1 - np.repeat(bin_msk.reshape((*bin_msk.shape, 1)), 3, axis=2))
