...
def read_node_matrix(
    ...
    # Variables to remove
    remove_vars = ['id', 'class']
    if remove_coords:
        remove_vars.extend(['X', 'Y'])
    if remove_area:
        remove_vars.extend(['area'])
    if remove_perimeter:
        remove_vars.extend(['perimeter'])
    if remove_std:
        remove_vars.extend(['std'])
    if remove_hist:
        remove_vars.extend([
    ...
    if return_class:
        ...
        # Class
        y = df['class'].to_numpy() - 1*(not enable_background)
        if stroma_mask is not None:
            y[y == stroma_label] = 0 + 1*enable_background        # stroma as negative class

        # Features
        X = df.drop(remove_vars, axis=1)
        X = X.to_numpy()
        if remove_prior and remove_coords and remove_area and remove_perimeter and remove_std and remove_hist:
            X = np.zeros((len(y), 1))

        # Coordinates information or not
        ...
        # Features
        remove_vars.remove('class')             # TODO: correct???
...
def _read_all_nodes(
    ...
    X, y = None, None
    for name in names:
        tmp = read_node_matrix(os.path.join(node_dir, name), remove_area=remove_area, remove_perimeter=remove_perimeter, remove_std=remove_std,
                                remove_hist=remove_hist, remove_prior=remove_prior, remove_coords=remove_coords, enable_background=enable_background)
        X_, y_ = tmp
        if X is None:
...
def read_all_nodes(
    ...
    X, y = _read_all_nodes(node_dir, names, remove_area=remove_area, remove_perimeter=remove_perimeter, remove_std=remove_std,
                            remove_hist=remove_hist, remove_prior=remove_prior)
    return X, y
